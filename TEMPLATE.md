...
> ## ⏸ PARKED — 2026-08-14
>
> **Do not merge this into `main`, and do not build on it without reading this
> first.** It is an archive, not active work.
>
> **Why it is parked.** LRH is the live deployment with real patients; a second
> adopting hospital is still hypothetical. PROJECT.md already sequences it that
> way — *"Now: a clean, reliable manual for LRH. Later (optional): a template
> other EDs can fork."* Landing a diff across every clinical file to serve a
> hypothetical fork, while the first hospital is still being dialed in, is the
> wrong order. Parked deliberately, not abandoned.
>
> **Branched from `main` at `b8ec9ba`** (2026-08-12). Main has moved on since.
>
> **What decayed and what did not.** Most of this branch is *new files* — the
> manifest, the generators, `ONBOARDING.md`, `strip_localization.mjs`,
> `local-config.js`, `localize-banner.js`, and the three new verify suites.
> Those cherry-pick cleanly no matter how far `main` moves. What decays is the
> mechanical strip (regenerated by re-running one script) and the hand
> conversions to `LOCAL()` in five tools: `clinical-pathways/heart/`,
> `arrest/`, `codes/`, `trauma/`, `tca/`. Reviving this is closer to half a day
> than to a rewrite.
>
> **To revive:**
> ```
> git checkout -B localization origin/main
> git checkout <this-branch> -- localization.manifest.mjs gen_onboarding.mjs \
>     gen_site_config.mjs strip_localization.mjs local-config.js \
>     localize-banner.js verify_localization.mjs verify_site_config.mjs
> node strip_localization.mjs        # rules + an audit of what they missed
> node gen_site_config.mjs && node gen_onboarding.mjs
> node build.mjs && bash run-tests.sh
> ```
> Then redo the five `LOCAL()` conversions against whatever those files look
> like by then, and check `strip_localization.mjs`'s two reports: rules that
> matched nothing, and lines that still name a site. Both should end empty.
>
> **Already salvaged onto `main` separately** (LRH value, no localization
> coupling): the seven new simulations, and the fixes to `verify_tca_screen.mjs`
> and `verify_kit_consistency.mjs`, which were both asserting against source
> files rather than the built output that actually ships.
