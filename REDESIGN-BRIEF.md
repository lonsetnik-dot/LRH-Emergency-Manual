...
3. `LOCALIZATION.md` — the full inventory of site-specific values and the risk
   tier of each. **31 items are safety-critical.** Read it before editing any
   file that contains a clinical value.
4. `CLAUDE.md` — the golden rules. **These outrank `DESIGN.md`.** If a design
...
5. `CASE-STATE.md` — the storage contract shared across tools.
6. `build.mjs` **and `demo-build.mjs`** — both read the shared stylesheets and
   scripts at module scope. Any change to a sheet's existence or name must be
   made in both, in the same commit.
7. `run-tests.sh` and the existing suites: **41 `verify_*.mjs` plus
...
- **The generic base ships no local truth, and no plausible default for anything
  safety-critical.** A removed Tier-1 value becomes an explicit `NOT SET` that
  **suppresses** the dependent display. Never a sensible-looking default — a
  hospital that hangs a different pressor concentration and never edits the
  config would read exactly half the rate it needs, in a confident large font,
  forever. This is the governing rule of the localization work.
- **Never invent a clinical value, a location, a phone number, or a review
  date.** If extraction leaves a hole, the hole is the correct output.
- **Never mix a clinical diff with a visual diff in the same commit.** A dose
  lost inside a two-thousand-line restyle is unreviewable. Localization
  extraction lands as its own commit, reviewed on clinical grounds, before any
  visual change to that file.
...
### Phase L — Generic base foundations

Runs after Phase 0, before any page migrates. This is **clinical extraction, not
design work**, and it is reviewed as such. `LOCALIZATION.md` is the specification.

1. **Decide and record the repository topology.** `main` becomes the generic
   base; LRH becomes a fork of it. Four documents currently give three different
   answers to this question (`CLAUDE.md` says no generic trunk exists,
   `site.config.json` says the same, `README.md` says the opposite,
   `LOCALIZING.md` already calls the project a generic trunk). Amend all four in
   one commit, or this whole effort has no stable ground.

2. **Build the `NOT SET` mechanism.** Extend the existing unset-weight pattern —
   which already renders `set weight` rather than a stale dose — into a general
   contract: any Tier-1 value that is unset suppresses its dependent display
   rather than defaulting it. Build this before removing anything, so removal
   lands into a safe renderer.

3. **Extend the config schema** for values that have nowhere to live today.
   Notably there is **no field for a blood bank / MTP activation number** — the
   manual says "say it, don't dial" and a fork has nowhere to put its number.
   Add the capability flags from `LOCALIZATION.md` Step 2 (cath lab, platelets
   on site, trauma level, neurosurgery, endoscopy/IR, obstetric service,
   tele-ED, tele-NICU, who may activate MTP), and author **both branches** of
   every capability rather than baking one in as prose.

4. **Fix the three highest-risk defects, named individually:**
   - The **hs-troponin mirror bug**. Ten threshold values are re-typed as plain
     text in `clinical-pathways/heart/`'s own reference tables, shadowing the
     SITE block. A fork that correctly edits the config still shows LRH's
     cutoffs in the table a clinician reads. Generate the tables from config.
   - The **pressor bag concentrations** in `codes/`, where a stale value halves
     every printed pump rate.
   - The **five duplicate-by-design config pairs** (`arrest/`↔`codes/` arrest
     values and routing triggers, NRP thresholds, cord timing,
     stale-weight window). A comment saying "change both by hand" is not a
     mechanism. Make one the source and derive the other, or test that they
     agree.

5. **Strip every clinician attribution, repo-wide.** Sixteen "CLINICALLY
   REVIEWED — [name]" assertions plus `reviewedBy` fields and signed-off names.
   A fork inheriting a named clinician's sign-off it never earned is a false
   credential on a bedside instrument. The fields stay; they ship empty; they
   are never backfilled — the same discipline `guidelines.js` already applies to
   `lastVerified`.

6. **Close the identity leaks** the config does not cover: the personal Gmail
   reached from 31 feedback buttons, 18 hardcoded upstream GitHub issue links,
   and the poison-control number hardcoded on one card while tokenized on two
   others (and US-specific either way).

7. **Wire the QR generator into the build** — this closes issues #28 and #29.
   Sixty-one codes in `labels/` pair a tokenized caption with a **static pixel
   matrix generated against LRH's domain**, so a label printed at another
   hospital shows *their* name and scans a clinician into *LRH's clinical
   values*. `generate_qr_codes.py` is referenced by nothing — not `build.mjs`,
   not `netlify.toml`, not `run-tests.sh` — and writes to `/tmp` for a human to
   paste back. The deliverable is **"the generator runs in `build.mjs`,"** not
   "codes regenerated once." Until then both issues reopen once per fork.
   Extend `verify_anchors.mjs` to assert each code points at *this* deployment,
   not merely that it decodes.

8. **Write the localization suites** (below), red first.

| Suite | Asserts |
|---|---|
| `verify_localization.mjs` | No site-specific string outside `site.config.json` and marked SITE CONFIG blocks. Every tool has a marked block. Fails while any Tier-1 value is unset — so a fork cannot go live half-localized. |
| `verify_config_mirror.mjs` | Any value appearing both in SITE CONFIG and in on-page text is *generated from* config, never re-typed. The troponin bug, generalized so it cannot recur. |
| `verify_config_pairs.mjs` | The five duplicate-by-design pairs agree. |
| `verify_attribution.mjs` | No clinician name, sign-off, or review date in the generic base. A fork's own name is permitted only where a matching date exists. |

**Then, in every phase below: step one is that phase's localization commit.**
Extract its files' site-specific values to config, land it, get it reviewed on
clinical grounds — and only then touch the markup. Tier-1 items concentrate in
`clinical-pathways/heart/`, `codes/`, `arrest/`, `peds/`, `pph/`, and `airway/`,
so Phases 3 through 5 carry most of this weight.
...
**Localization step for this phase is large but low-risk** — it is almost all
Tier 2. `labels/` holds 75 label objects, 18 location strings, and hardware
dimensions measured from LRH's actual drawer fittings; nine posters print a
location banner. All of it becomes config. Note that stripping locations to
`LOCATION NOT SET` changes string lengths, which can change pagination — so the
localization commit and the poster-pagination check run together here.
...
### Phase 6b — Iconography pass (closes issue #36)

The pacemaker/ICD card (`codes/#c26`) carries no glyph at all, while the magnet
silhouette in `equipment-icons.js` reaches the label sheet and never the card.
Inject it via the `/* @icons */` marker — equipment glyphs are objects and belong
to that registry, never pasted — and add a rendered-markup assertion covering
card-versus-label parity.

This is also where the human review of glyph discriminability at 2 m and 16px is
scheduled, now that location rides on glyphs rather than color.
...
- Amend `LAYOUT.md`: remove the self-declared failing state; the two-breakpoint
  rule is now tested.
- Replace **`LOCALIZING.md` and `LOCALIZATION-WORKSHEET.md`** with
  `LOCALIZATION.md`. Both currently list `--d1`–`--d6` as localizable fields, and
  `LOCALIZING.md`'s "seven things" section understates the real surface by a
  wide margin — the sweep found 64 items across three risk tiers, not seven
  categories. Both also contain LRH's own worked answers, which a fork editing
  in place would publish as its own.
- Confirm the topology amendments from Phase L landed consistently across
  `CLAUDE.md`, `SITES.md`, `README.md`, and `site.config.json`'s comment block.
...
16. Every QR destination and cross-tool anchor resolves in `dist/`, and every QR
    matrix is generated from `site.config.json` rather than baked.
17. Impeccable's detector clean, or suppressed with a written reason.

**Generic-base criteria** — the fork test:

18. No site-specific string outside `site.config.json` and marked SITE CONFIG
    blocks. Every tool has a marked block.
19. Every Tier-1 value ships `NOT SET` in the base, and its dependent display is
    suppressed rather than defaulted. No plausible default anywhere.
20. Zero clinician names, sign-offs, or review dates in the generic base.
21. No value appears both in config and as re-typed on-page text.
22. The five duplicate-by-design pairs are derived from one source, or tested to
    agree.
23. Every capability claim that changes clinical behavior — cath lab, platelets,
    trauma level, neurosurgery, endoscopy, obstetric service, tele-services — is
    a config flag with both branches authored, not prose.
24. **The real test:** a clinician can take the base, walk `LOCALIZATION.md`, and
    stand up a working manual for their own department without editing logic.
...
- **Do not restructure clinical content.** Words, order, thresholds, and citations
  are reviewed. Extraction moves a value from markup to config; it never changes
  the value.
- **Do not invent anything to fill a hole.** Not a dose, not a location, not a
  phone number, not a review date. An empty Tier-1 field is the correct output
  of extraction. Inventing a review date is fabricating a clinical credential.
- **Do not ship a plausible default.** The whole failure mode is a value that
  looks right and is wrong for the hospital reading it.
- **Do not mix a clinical diff with a visual diff.** Separate commits, always.
